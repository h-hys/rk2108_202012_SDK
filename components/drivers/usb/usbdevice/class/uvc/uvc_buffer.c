...
static void uvc_buffer_irq(struct uvc_device *uvc)
{
    ...
    if (!intsts)
    {
         ...
         * the buffer manager (BM), so route the irq to another block when
         * the current uvc block interrupt status is zero, and the BM driver
         * will handle to process it.
         */
...
uvc_buffer_init(struct uvc_device *uvc)
{
    ...
    {
    case UVC_ID_DEPTH:
        uvc_buffer_var_init(video, BUF_MGR_WCH2, BUF_MGR_WCH2, BUF_MGR_WCH_IRQ_DEPTH);
        handler = (rt_isr_handler_t)uvc_buffer_irq_0;
        break;
    case UVC_ID_RGB:
        uvc_buffer_var_init(video, BUF_MGR_WCH1, BUF_MGR_BLK0, BUF_MGR_WCH_IRQ_RGB);
        handler = (rt_isr_handler_t)uvc_buffer_irq_1;
        break;
    case UVC_ID_IR:
        uvc_buffer_var_init(video, BUF_MGR_WCH3, BUF_MGR_BLK0, BUF_MGR_WCH_IRQ_IR);
