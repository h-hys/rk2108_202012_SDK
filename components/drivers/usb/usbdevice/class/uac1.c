...
static rt_err_t _audio_set_endpoint_req(ufunction_t func, ureq_t setup)
{
    ...
    if (w_value == UAC_EP_CS_ATTR_SAMPLE_RATE << 8)
    {
        ...
        case UAC_SET_CUR:
            if (len > 0)
            {
                if (intf == F_AUDIO_AS_OUT_INTERFACE)
...
static rt_err_t _audio_get_endpoint_req(ufunction_t func, ureq_t setup)
{
    ...
    rt_uint32_t rate = (intf == F_AUDIO_AS_OUT_INTERFACE ? g_uac1->p.sample_rate :
...
static void _audio_start_capture(ufunction_t func)
{
    ...
    ep_in->request.size = g_uac1->c.sample_rate * C_NR_CHANNEL * (C_AUDIO_SAMPLE_BITS >> 3) / 1000;
...
static rt_err_t _ep_in_handler(ufunction_t func, rt_size_t size)
{
    ...
    size = c->sample_rate * C_NR_CHANNEL * (C_AUDIO_SAMPLE_BITS >> 3) / 1000;
    if (c->sample_rate == 44100 && ++c->frame_cnt % 10 == 0)
    {
        size = c->sample_rate * C_NR_CHANNEL * (C_AUDIO_SAMPLE_BITS >> 3) / 100 - 9 * size;
        c->frame_cnt = 0;
    }
