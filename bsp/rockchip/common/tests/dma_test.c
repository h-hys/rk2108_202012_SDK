...
{
    struct rt_dma_transfer *xfer = (struct rt_dma_transfer *)param;
    rt_int32_t ret;

    ret = rt_memcmp((void *)xfer->src_addr, (void *)xfer->dst_addr, xfer->len);
    RT_ASSERT(ret == 0);
...
void dma_test(void)
{
    ...
    rt_kprintf("this is simple dma_test\n");
