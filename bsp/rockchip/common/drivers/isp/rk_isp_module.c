                               ...
                               bool flag, struct rk_isp_dpcc_config *arg)
{
    ...
    ret = rt_mutex_take(isp_dev->isp_mutex, RT_WAITING_FOREVER);
...
static void rt_do_dump_rkisp_reg(struct rt_rkisp_isp_dev *isp_dev, int level)
{
#ifdef RKISP_DEBUG_REG
    if (isp_dev->dpcc_en)
...
void rt_do_rkisp_v_start(struct rt_rkisp_isp_dev *isp_dev,
                         const rt_tick_t *timestamp)
{
    unsigned int write_id;
    /* Called in an interrupt context. */
    isp_dev->frame_id += 2;
...
void rt_do_disable_rkisp(struct rt_rkisp_isp_dev *isp_dev)
{
    ...
    rt_mutex_take(isp_dev->isp_mutex, RT_WAITING_FOREVER);
    ...
    struct rt_rkisp_isp_dev *isp_dev, unsigned int send_meas)
{
    ...
    if (active_meas & RK_ISP_AWB_DONE)
    {
        ...
        stat_buf->meas_type |= RK_ISP_STAT_AWB;
    }
    if (active_meas & RK_ISP_AFM_FIN)
    {
        ...
        stat_buf->meas_type |= RK_ISP_STAT_AFM_FIN;
    }
    if (active_meas & RK_ISP_EXP_END)
    {
        ...
        stat_buf->meas_type |= RK_ISP_STAT_AUTOEXP;
    }
    if (active_meas & RK_ISP_HIST_MEASURE_RDY)
    {
        ...
        stat_buf->meas_type |= RK_ISP_STAT_HIST;
    }
                                ...
                                unsigned int send_meas, bool force_send)
{
    ...
    if (send_meas & RK_ISP_AWB_DONE)
        module_updates |= RK_ISP_MODULE_AWB;
    if (send_meas & RK_ISP_AFM_FIN)
        module_updates |= RK_ISP_MODULE_AFC;
    if (send_meas & RK_ISP_EXP_END)
        module_updates |= RK_ISP_MODULE_AEC;
    if (send_meas & RK_ISP_HIST_MEASURE_RDY)
        module_updates |= RK_ISP_MODULE_HST;

    if (((isp_dev->meas_cfgs.module_updates & module_updates) == 0 &&
            (isp_dev->active_meas & send_meas)) || force_send)
    {
        rt_do_send_rkisp_measurement(isp_dev, module_updates);
    }
    return 0;
}
...
void rt_do_get_meas_data_buf(struct rt_rkisp_isp_dev *isp_dev, void *arg)
{
    ...
    isp_metadata[1] = &rk_isp_dev->isp_buf_metadata[rk_isp_dev->buf_index];

    if (isp_metadata[0]->frame_id > isp_metadata[1]->frame_id)
        arg = (void *)&isp_metadata[0]->isp_meta;
    else
        arg = (void *)&isp_metadata[1]->isp_meta;
}
...
int rt_do_set_rkisp_isp_module_interrupt(struct rt_rkisp_isp_dev *isp_dev, unsigned int isp_mis)
{
    ...
    {
        ISP_DBG("handle get hist meas data\n");
        isp_dev->hst_meas_ready = true;
        rt_rkisp_get_hst_meas(isp_dev, &isp_dev->meas_stats.stat);
    }

    if ((isp_dev->meas_send_alone & RK_ISP_AWB_DONE) && isp_dev->awb_meas_ready)
    {
        isp_dev->awb_meas_ready = false;
        rt_do_rkisp_meas_queue_work(isp_dev, RK_ISP_AWB_DONE, false);
    }
    if ((isp_dev->meas_send_alone & RK_ISP_AFM_FIN) && isp_dev->afm_meas_ready)
    {
        isp_dev->afm_meas_ready = false;
        rt_do_rkisp_meas_queue_work(isp_dev, RK_ISP_AFM_FIN, false);
    }
    if ((isp_dev->meas_send_alone & (RK_ISP_EXP_END | RK_ISP_HIST_MEASURE_RDY))
            && isp_dev->aec_meas_ready && isp_dev->hst_meas_ready)
    {
        isp_dev->aec_meas_ready = false;
        isp_dev->hst_meas_ready = false;
        rt_do_rkisp_meas_queue_work(isp_dev,
                                    RK_ISP_EXP_END | RK_ISP_HIST_MEASURE_RDY, false);
    }
    ...
    if (isp_mis & RK_ISP_FRAME)
    {
        ...
        if (!(isp_dev->meas_send_alone & RK_ISP_AWB_DONE))
            isp_dev->awb_meas_ready = false;
        if (!(isp_dev->meas_send_alone & RK_ISP_AFM_FIN))
            isp_dev->afm_meas_ready = false;
        if (!(isp_dev->meas_send_alone & RK_ISP_EXP_END))
            isp_dev->aec_meas_ready = false;
        if (!(isp_dev->meas_send_alone & RK_ISP_HIST_MEASURE_RDY))
            isp_dev->hst_meas_ready = false;
