...
static struct rk_isp_device g_rk_isp_instance;
...
enum rk_isp_pix_fmt rt_do_rk_isp_fmt_trans(unsigned int v4l2_pix_fmt)
{
    ...
    case V4L2_PIX_FMT_JPEG:
        return RK_JPEG;
    default:
        rt_kprintf("unknown or unsupported V4L2 pixel format %c%c%c%c\n",
    ...
    struct rk_isp_device *dev)
{
    ...
    for (int k = 0; k < count; k++)
    {
        ...
        {
            rt_kprintf("Err: Can't find isp subdev:%s\n", subdev_name[k].data);
        }
    }
    ...
    struct rk_isp_device *dev)
{
    ...
    if (!(sub_dev))
    {
        rt_kprintf("no image source selected as input (call s_input first)\n");
        ret = -RT_ERROR;
        goto err;
    }
...
#if defined(RT_USING_CAMERA) && RT_USING_CAMERA_CMD
    ret = subdev->ops->control(dev->subdev, RK_ISP_CID_VBLANKING, &vblanking);
    if (IS_ERR_VALUE(ret))
    {
        ISP_DBG("get vblanking failed: %d\n", ret);
        vblanking = 0;
    }
...
static void rt_do_config_rkisp_ism(struct rk_isp_device *dev, bool async)
{
    ...
    else
    {
        ...
        WRITE_RKISP_REG(pconfig->ism_params.max_dy, RK_ISP_IS_MAX_DY);
        WRITE_RKISP_REG(pconfig->ism_params.displace, RK_ISP_IS_DISPLACE);
        WRITE_RKISP_REG(0, RK_ISP_IS_H_OFFS);
        WRITE_RKISP_REG(0, RK_ISP_IS_V_OFFS);
        WRITE_RKISP_REG(dev->config.isp_config.output.width, RK_ISP_IS_H_SIZE);
        WRITE_RKISP_REG(dev->config.isp_config.output.height, RK_ISP_IS_V_SIZE);
    ...
    struct rk_isp_device *dev)
{
    ...
    ret = sub_dev->ops->control(
              sub_dev,
              RK_DEVICE_CTRL_CAMERA_SENSOR_MODE_DATA,
              &sensor_mode);
#else
    ...
    struct rk_isp_device *dev)
{
    ...
    if (IS_ERR_VALUE(ret))
    {
        rt_kprintf("ioctl PLTFRM_RKCAM_G_ITF_CFG failed!\n");
        goto err;
    }
    ...
    struct rk_isp_device *dev)
{
            ...
            RKISP_PIX_FMT_IS_Y_ONLY(in_pix_fmt))
    {
        ...
        {
            rt_kprintf("%d bits per pixel not supported\n", bpp);
            ret = -RT_EINVAL;
            goto err;
        }
        ...
        if (RKISP_PIX_FMT_IS_RAW_BAYER(in_pix_fmt))
        {
            ...
            {
                rt_kprintf("BAYER pattern not supported\n");
                ret = -RT_EINVAL;
                goto err;
            }
        }
    ...
    else if (RKISP_PIX_FMT_IS_YUV(in_pix_fmt))
    {
        ...
        if (dev->config.input_sel == RK_ISP_INP_DMA)
        {
            ...
            {
                rt_kprintf("format %s not supported, invalid bpp %d\n",
        ...
        else
        {
            ...
            else if (PLTFRM_CAM_ITF_IS_DVP(dev->config.cam_itf.type))
            {
                ...
                case 12:
                    ...
                    break;
                default:
                    rt_kprintf("%s isn't support for RK isp11\n",
    ...
    {
        rt_kprintf("format %s not supported\n",
    ...
    output->width = 1920;
    output->height = 1080;
    WRITE_RKISP_REG(signal, RK_ISP_ACQ_PROP);
    WRITE_RKISP_REG(0, RK_ISP_ACQ_H_OFFS);
    WRITE_RKISP_REG(0, RK_ISP_ACQ_V_OFFS);
    WRITE_RKISP_REG(1920, RK_ISP_ACQ_H_SIZE);
    WRITE_RKISP_REG(1080, RK_ISP_ACQ_V_SIZE);
    WRITE_RKISP_REG(0, RK_ISP_OUT_V_OFFS);
    WRITE_RKISP_REG(0, RK_ISP_OUT_H_OFFS);
    WRITE_RKISP_REG(output->width, RK_ISP_OUT_H_SIZE);
    WRITE_RKISP_REG(output->height, RK_ISP_OUT_V_SIZE);
    WRITE_RKISP_REG(0, RK_ISP_IMSC);
    WRITE_RKISP_REG(0, RK_ISP_ACQ_NR_FRAMES);
    WRITE_RKISP_REG(0x00000af7, RK_ISP_CTRL);
#else
    ...
    WRITE_RKISP_REG(v_offs, RK_ISP_OUT_V_OFFS);
    WRITE_RKISP_REG(h_offs, RK_ISP_OUT_H_OFFS);
    WRITE_RKISP_REG(output->width, RK_ISP_OUT_H_SIZE);
    WRITE_RKISP_REG(output->height, RK_ISP_OUT_V_SIZE);
#endif
    ...
    struct rk_isp_device *dev)
{
    ...
    {
        rt_kprintf("invalid number (%d) of MIPI lanes, valid range is [1..4]\n",
                   dev->config.cam_itf.cfg.mipi.nb_lanes);
        ret = -RT_EINVAL;
        goto err;
    }
...
#ifndef FPGA_TEST
    mipi_ctrl =
        RKMIPI_CTRL_NUM_LANES(
            dev->config.cam_itf.cfg.mipi.nb_lanes - 1) |
        RKMIPI_CTRL_SHUTDOWNLANES(shutdown_lanes) |
        RK_MIPI_CTRL_ERR_SOT_SYNC_HS_ENA |
        RK_MIPI_CTRL_ERR_SOT_HS_ENA |
        RK_MIPI_CTRL_CLOCKLANE_ENA;

    WRITE_RKISP_REG(mipi_ctrl, RK_MIPI_CTRL);
#else
    mipi_ctrl = 0x00000000;
    WRITE_RKISP_REG(mipi_ctrl, RK_MIPI_CTRL);
    mipi_ctrl = 0x00040000;
    WRITE_RKISP_REG(mipi_ctrl, RK_MIPI_CTRL);
    mipi_ctrl = 0x00040f01;
    WRITE_RKISP_REG(mipi_ctrl, RK_MIPI_CTRL);
    WRITE_RKISP_REG(0x01, 0x40176044);
    WRITE_RKISP_REG(0x02, 0x40176040);
    WRITE_RKISP_REG(0x01, 0x40176040);
#endif

    /* mipi_dphy */
    WRITE_RKISP_REG(0x04000400, 0x40240208);
    WRITE_RKISP_REG(0x04000000, 0x40240208);
    WRITE_RKISP_REG(0x00030003, 0x4024020C);
    ...
    struct rk_isp_device *dev)
{
    ...
    if (RKISP_PIX_FMT_IS_YUV(out_pix_fmt))
    {
        ...
        {
            rt_kprintf("wrong num_cplanes %s for y12 path\n",
                       rk_isp_pix_fmt_string(out_pix_fmt));
            return -RT_EINVAL;
        }
    ...
    {
        rt_kprintf("wrong format %s for y12 path\n",
                   rk_isp_pix_fmt_string(out_pix_fmt));
        return -RT_EINVAL;
    }
    ...
    struct rk_isp_device *dev)
{
    ...
    if (!RKISP_PIX_FMT_IS_YUV(in_pix_fmt))
    {
        rt_kprintf("unsupported format %s (must be YUV)\n",
                   rk_isp_pix_fmt_string(in_pix_fmt));
        ret = -RT_EINVAL;
        goto err;
    }
    ...
    if (RKISP_PIX_FMT_IS_YUV(out_pix_fmt))
    {
        ...
        {
            rt_kprintf("unsupported YUV output format %s\n",
                       rk_isp_pix_fmt_string(out_pix_fmt));
            ret = -RT_EINVAL;
            goto err;
        }
    ...
    else if (RKISP_PIX_FMT_IS_RGB(out_pix_fmt))
    {
        ...
        {
            rt_kprintf("unsupported RGB output format %s\n",
                       rk_isp_pix_fmt_string(out_pix_fmt));
            ret = -RT_EINVAL;
            goto err;
        }
    ...
    {
        rt_kprintf("unsupported output format %s\n",
                   rk_isp_pix_fmt_string(out_pix_fmt));
        ret = -RT_EINVAL;
        goto err;
    }
    ...
    {
        rt_kprintf("unsupported YUV input format %s\n",
                   rk_isp_pix_fmt_string(in_pix_fmt));
        ret = -RT_EINVAL;
        goto err;
    }
    ...
    struct rk_isp_device *dev)
{
    ...
    if (RKISP_PIX_FMT_IS_YUV(out_pix_fmt))
    {
        ...
        {
            ISP_DBG("unsupported YUV output format %s\n",
                    rk_isp_pix_fmt_string(out_pix_fmt));
            ret = -RT_EINVAL;
            goto err;
        }
    ...
    {
        ISP_DBG("unsupported output format %s\n",
                rk_isp_pix_fmt_string(out_pix_fmt));
        ret = -RT_EINVAL;
        goto err;
    }
    ...
    unsigned int stream_ids)
{
    ...
    else
    {
        ...
        {
            ISP_DBG("Sensor Interface: 0x%x isn't support \n",
                    dev->config.cam_itf.type);
            return -RT_EINVAL;
        }
    }
    ...
    bool async)
{
    ...
    if (RKISP_PIX_FMT_IS_YUV(rsz_output->pix_fmt))
    {
        ...
        {
            ISP_DBG("input is black and white, cannot output colour\n");
            ret = -RT_EINVAL;
            goto err;
        }
    }
    else
    {
        ...
        {
            ISP_DBG("%dx%d -> %dx%d isn't support,"
    ...
    unsigned int stream_ids)
{
            ...
            (stream_ids & RK_ISP_STREAM_Y12))
    {
        ...
        if (IS_ERR_VALUE(ret))
            rt_kprintf("set stream path failed\n");
    }
    ...
    enum rk_isp_stream_id stream_id)
{
    ...
    case RK_ISP_STREAM_DMA:
        ...
        break;
    default:
        ISP_DBG("unknown/unsupported stream ID %d\n", stream_id);
        break;
    }
    ...
    unsigned int stride)
{
    ...
    {
        ISP_DBG("Y12 path don't support format %s,"
    ...
    unsigned int stride)
{
    ...
    if (dev->config.mi_config.raw_enable)
        ISP_DBG("cannot activate SP when MP is set to RAW data output, you will not be able to (re-)start streaming\n");
    ...
    {
        ISP_DBG("format %s %dx%d@%d/%dfps, stride = %d not supported on SP\n",
                rk_isp_pix_fmt_string(strm_fmt->frm_fmt.pix_fmt),
                strm_fmt->frm_fmt.width,
                strm_fmt->frm_fmt.height,
                strm_fmt->frm_intrvl.numerator,
    ...
    unsigned int stride)
{
    ...
    {
        ISP_DBG("format %s %dx%d@%d/%dfps, stride = %d not supported for DMA\n",
                rk_isp_pix_fmt_string(strm_fmt->frm_fmt.pix_fmt),
                strm_fmt->frm_fmt.width,
                strm_fmt->frm_fmt.height,
                strm_fmt->frm_intrvl.numerator,
    ...
    if (stop_sp)
    {
        ...
        if (g_data_stream.state == RK_ISP_STATE_STREAMING)
        {
            ...
            if (dev->config.mi_config.data_path.busy)
                rt_kprintf("DATA PATH STILL STREAMING WHILE STOPPING IT\n");
        }
...
#if defined(RT_USING_CAMERA) && RK_USING_CMD
        ret = sub_dev->ops.control(
                  sub_dev,
                  RK_VIDIOC_SENSOR_MODE_DATA,
                  &dev->subdev_exps.data[0].data);
        )
#endif
    ...
    if (!RKISP_MI_IS_BUSY(dev))
    {
        ...
        if (dev->b_isp_frame_in &&
                (g_data_stream.state == RK_ISP_STATE_STREAMING) && ((dev->rk_isp_data_path == RK_ISP_STREAM_MP) ||
                        (dev->rk_isp_data_path == RK_ISP_STREAM_SP)))
    ...
    struct rk_isp_device *dev)
{
    ...
    if ((streamon_sp || streamon_mp || streamon_y12) &&
            (g_data_stream.state != RK_ISP_STATE_READY))
    {
        rt_kprintf("cannot start streaming on the current path, path not yet enabled\n");
        ret = -RT_ERROR;
        goto err;
    }
...
int rt_do_rkisp_streamoff(struct rk_isp_device *dev)
{
    ...
    return 0;
err:
    rt_kprintf("Stream state = %s, # frames received = %d\n",
               rk_isp_state_string(g_data_stream.state),
               dev->isp_dev.frame_id >> 1);
    rt_kprintf("failed with error %d\n", ret);
    return ret;
    ...
    unsigned int stride)
{
    ...
    default:
        rt_kprintf("unknown/unsupported stream ID %d\n", stream_id);
        ret = -RT_EINVAL;
        goto err;
    }
    ...
    unsigned int stream_ids)
{
    ...
    {
        rt_kprintf("unknown/unsupported stream IDs 0x%08x\n",
                   stream_ids);
        ret = -RT_EINVAL;
        goto err;
    }
    ...
    return 0;
err:
    rt_kprintf("failed with error %d in func %s\n", ret, __FUNCTION__);
    return ret;
}
    ...
    struct rk_isp_device *dev)
{
    ...
    if (!dev->b_stream_on)
    {
        rt_kprintf("stream is not streaming\n");
        return -1;
    }
    ...
    unsigned int *target_height)
{
    ...
    {
        rt_kprintf("cannot get target frame size, no path ready\n");
        return -RT_ERROR;
    }
    return 0;
}
    ...
    unsigned int *v_offs)
{
    ...
    {
        rt_kprintf("no input selected for ISP\n");
        ret = -RT_ERROR;
        goto err;
    }
    ...
    unsigned int *size)
{
    ...
    {
        rt_kprintf("cannot calculate buffer size, %s stream not ready\n",
                   rk_isp_stream_id_string(stream_id));
        ret = -RT_EINVAL;
        goto err;
    }
                                  ...
                                  unsigned int width, unsigned int *stride)
{
    ...
    case V4L2_PIX_FMT_GREY:
        *stride = width;
        break;
    default:
        rt_kprintf("%s: Unsupported V4L2 pixel format %c%c%c%c\n",
                   __FUNCTION__,
...
void rt_do_rkisp_mipi_isr(int mipi_mis, void *cntxt)
{
    ...
    {
        rt_kprintf("RK_MIPI_ERR_DPHY: 0x%x\n", mipi_mis);
    ...
    {
        rt_kprintf("RK_MIPI_ERR_CSI: 0x%x\n", mipi_mis);
    }

    if (mipi_mis & RKMIPI_SYNC_FIFO_OVFLW(3))
    {
        rt_kprintf("RK_MIPI_SYNC_FIFO_OVFLW: 0x%x\n", mipi_mis);
    }

    if (mipi_mis & RK_MIPI_ADD_DATA_OVFLW)
    {
        rt_kprintf("RK_MIPI_ADD_DATA_OVFLW: 0x%x\n", mipi_mis);
    }
...
void rt_do_rkisp_isr(int mis, void *cntxt)
{
    ...
    if ((isp_mis & (RK_ISP_DATA_LOSS | RK_ISP_PIC_SIZE_ERROR)))
    {
        ...
        if ((isp_mis & RK_ISP_PIC_SIZE_ERROR))
        {
            ...
            rt_kprintf("RK_ISP_PIC_SIZE_ERROR (0x%08x)", isp_err);
    ...
    if (isp_mis & RK_ISP_FRAME_IN)
    {
        ...
        // rt_do_rkisp_frame_in(&dev->isp_dev, &tv);
        isp_mis_tmp = READ_RKISP_REG(RK_ISP_MIS);
        if (isp_mis_tmp & RK_ISP_FRAME_IN)
            rt_kprintf("isp icr frame_in err: 0x%x\n", isp_mis_tmp);
    ...
    if (isp_mis & RK_ISP_FRAME)
    {
        ...
        if (isp_mis_tmp & RK_ISP_FRAME)
            rt_kprintf("isp icr frame end err: 0x%x\n", isp_mis_tmp);
    ...
    rt_do_set_rkisp_isp_module_interrupt(&dev->isp_dev, isp_mis);
    rk_isp_function_exit();
    rt_interrupt_leave();
    return;
}
...
rt_err_t rk_isp_init(struct rk_isp_dev *dev)
{
    ...
    if (ret)
    {
        rt_kprintf("rt_do_init_rkisp_subdev failed, plz check\n");
        return RT_ERROR;
    }
    else
    {
    ...
    if (ret)
    {
        rt_kprintf("rt_do_get_rkisp_stride failed, plz check\n");
        return RT_ERROR;
    }
    ...
    if (ret)
        ISP_DBG("rt_do_rkisp_init failed, plz check\n");
    else
...
rt_err_t rk_isp_open(struct rk_isp_dev *dev, unsigned short oflag)
{
    ...
    if (!ispdev->mutex)
    {
        ...
        if (!ispdev->mutex)
        {
            rt_kprintf("create queue queued mutex failed!\n");
            return -RT_ENOMEM;
        }
    }
...
rt_err_t rk_isp_close(struct rk_isp_dev *dev)
{
    ...
    if (ret)
    {
        rt_kprintf("rk isp data path is streaming, can not close\n");
        return RT_ERROR;
    }
    ...
    {
        rt_kprintf("Warning: isp maybe has been closed!\n");
        return RT_ENOSYS;
    }
...
rt_err_t rk_isp_control(struct rk_isp_dev *dev, int cmd, void *args)
{
    ...
    case RK_ISP_CMD_IMAGE_EFFECT:
        ...
        {
            rt_kprintf("unknown/unsupported image effect %d\n", data[0]);
            return -RT_EINVAL;
        }
    ...
    case RK_ISP_CMD_SET_ISP_INIT_INFO:
        ...
        rt_kprintf("isp config init param success\n");
        ret = 0;
        break;
