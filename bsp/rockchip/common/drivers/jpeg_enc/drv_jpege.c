...
static rt_err_t update_header_and_regs(void *ctx)
{
    ...
    if (cfg->cfg_change)
    {
        ...
        rk_cal_buf_size(cfg);
        jpege_bits_setup(jpege_ctx->bits, cfg->jpeg_header_addr + 4, 1024);
        jpeg_write_header(jpege_ctx);
        jpege_dbg("jpege header write %d bytes done\n", jpege_bits_get_bytepos(jpege_ctx->bits));
