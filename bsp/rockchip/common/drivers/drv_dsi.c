...
static void rockchip_dsi_enable(struct display_state *state)
{
    ...
    /**
     * The command of set brightness and area are sent in HS mode,
     * So need change to HS mode when panel init in LP mode.
     */
#if defined(RT_HW_LCD_DCS_BACKLIGHT) || defined(RT_HW_LCD_AREA_DISPLAY)
