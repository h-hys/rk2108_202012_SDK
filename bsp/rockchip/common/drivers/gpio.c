...
static int gpio_set(int argc, char **argv)
{
    ...
    if (!strcmp(val, "i"))
    {
        HAL_PINCTRL_SetIOMUX(bank, HAL_BIT(pin), 0);
        HAL_GPIO_SetPinDirection(pGPIO, HAL_BIT(pin), GPIO_IN);
    }
    else if (!strcmp(val, "o"))
    {
        HAL_PINCTRL_SetIOMUX(bank, HAL_BIT(pin), 0);
        HAL_GPIO_SetPinDirection(pGPIO, HAL_BIT(pin), GPIO_OUT);
    }
    else if (!strcmp(val, "h"))
    {
        HAL_PINCTRL_SetIOMUX(bank, HAL_BIT(pin), 0);
        HAL_GPIO_SetPinDirection(pGPIO, HAL_BIT(pin), GPIO_OUT);
        HAL_GPIO_SetPinLevel(pGPIO, HAL_BIT(pin), GPIO_HIGH);
    }
    else if (!strcmp(val, "l"))
    {
        HAL_PINCTRL_SetIOMUX(bank, HAL_BIT(pin), 0);
        HAL_GPIO_SetPinDirection(pGPIO, HAL_BIT(pin), GPIO_OUT);
        HAL_GPIO_SetPinLevel(pGPIO, HAL_BIT(pin), GPIO_LOW);
    }
    else
    {
