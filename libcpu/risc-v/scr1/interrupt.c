...
RT_WEAK rt_uint32_t rt_hw_interrupt_get_active(void)
{
    rt_uint32_t cisv, irq;

    cisv = read_csr_ipic(IPIC_CISV);
	irq = cisv == MAX_INTERRUPT_VECTOR ? NUM_INTERRUPTS : cisv;
	return irq;
}
...
static void dump_csr_ipic()
{
    rt_uint32_t cisv, cicsr, ipr, isvr, icsr;

    cisv = read_csr_ipic(IPIC_CISV);
	cicsr = read_csr_ipic(IPIC_CICSR);
	ipr = read_csr_ipic(IPIC_IPR);
	isvr = read_csr_ipic(IPIC_ISVR);
	icsr = read_csr_ipic(IPIC_ICSR);
	rt_kprintf("cisv=%d, cicsr=%d, ipr=%d, isvr=%d, icsr=%d\n", cisv, cicsr, ipr, isvr, icsr);
}
